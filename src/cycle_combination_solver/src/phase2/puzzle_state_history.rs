...
pub trait PuzzleStateHistoryBuf<P: PuzzleState>:
    ValidPuzzleStateHistoryBuf<P> + Index<usize, Output = P> + IntoIterator<Item = P>
{
    fn initialize(puzzle_def: &PuzzleDef<P>) -> Self;
    fn push_stack(&mut self, puzzle_state: &P, stack_index: usize, puzzle_def: &PuzzleDef<P>);
}
...
impl<P: PuzzleState, B: PuzzleStateHistoryBuf<P>> PuzzleStateHistory<P, B> {
    pub fn push_stack(&mut self, puzzle_state: &P, puzzle_def: &PuzzleDef<P>) {
        self.stack
            .push_stack(puzzle_state, self.stack_index, puzzle_def);
        self.stack_index += 1;
    }
        ...
        for i in 1..=self.stack_index {
            ...
            move_sequence.push(
                puzzle_def
                    .moves
                    .iter()
                    .find(|m| m.puzzle_state == result)
                    .expect("Move sequence is not valid")
                    .clone(),
            );
        }
...
impl<P: PuzzleState> PuzzleStateHistoryBuf<P> for Vec<P> {
    ...
    fn push_stack(&mut self, puzzle_state: &P, stack_index: usize, puzzle_def: &PuzzleDef<P>) {
        ...
        if stack_index + 1 >= self.len() {
            ...
            // index
            let last_entry = unsafe { left.get_unchecked(stack_index) };
            next_entry.replace_compose(last_entry, puzzle_state, &puzzle_def.sorted_orbit_defs);
        }
    }
}
...
mod tests {
    ...
    fn puzzle_state_history_composition<B: PuzzleStateHistoryBuf<Cube3>>() {
        ...
        puzzle_state_history.push_stack(&r_move.puzzle_state, &cube3_def);
        puzzle_state_history.push_stack(&r_move.puzzle_state, &cube3_def);
    ...
    fn puzzle_state_history_pop<B: PuzzleStateHistoryBuf<Cube3>>() {
        ...
        puzzle_state_history.push_stack(&r_move.puzzle_state, &cube3_def);
        puzzle_state_history.push_stack(&r2_move.puzzle_state, &cube3_def);
        ...
        puzzle_state_history.push_stack(&f2_move.puzzle_state, &cube3_def);
        ...
        puzzle_state_history.push_stack(&f2_move.puzzle_state, &cube3_def);
        puzzle_state_history.push_stack(&r_prime_move.puzzle_state, &cube3_def);
