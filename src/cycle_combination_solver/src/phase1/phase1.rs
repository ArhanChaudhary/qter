...
#[derive(Clone)]struct PrimeCombo {
    order: usize,
    values: Vec<usize>,
    piece_total: usize,
...
fn efficient_cycle_combinations(
    ...
    let mut cycle_cubie_counts: Vec<usize> = vec![0;num_registers];
    ...
    let possible_orders: Vec<PrimeCombo> = possible_order_list(
        &cubies_per_register, 
        cycle_cubie_counts.iter().max().unwrap().min(&cubies_per_register), 
        &max_orient);
    ...
    for order in possible_orders{
        ...
        for o in order.values.iter().enumerate(){
            if o.1 % 2 == 0{
                let orientable = (max_orient[2]/1.max(order.piece_counts[o.0])).min(num_registers);
                unorientable_excess += (num_registers - orientable) * (o.1 - order.piece_counts[o.0]);
            } else if o.1 % 3 == 0 {
                let orientable = (max_orient[3]/1.max(order.piece_counts[o.0])).min(num_registers);
                unorientable_excess += (num_registers - orientable) * (o.1 - order.piece_counts[o.0]);
            } 
        }

        if unorientable_excess + num_registers * order.piece_counts.iter().sum() > total_cubies{
            continue;
        }
        ...
        if assignments.len() > 0 {
            return Some(assignments_to_combo(&assignments, 
                &vec![order.clone();num_registers], 
                &cycle_cubie_counts, 
                puzzle_orbit_definition
            ))
...
fn main() {
    ...
    let cycle_combos = efficient_cycle_combinations(
        &puzzle_orbit_definition, 
        2);

    println!("{}",cycle_combos.unwrap().cycles[0].order);
}
